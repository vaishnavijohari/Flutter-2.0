...
// --- UPDATED: Import the new models and detail screen ---
import '../models.dart';
import 'story_detail_screen.dart';


// --- UPDATED: Local models now include an ID for navigation ---
// We keep these local models because they have unique properties (views, chapter, etc.)
class TrendingStory {
  final String id;
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchHomePageData() async {
    try {
      ...
      setState(() {
        // --- UPDATED: Dummy data now creates objects with unique IDs ---
        _newlyAddedStories = List.generate(5, (i) => Story(id: 'newly-$i', imageUrl: 'assets/images/story_$i.jpg', title: 'Newly Added Story ${i+1}'));
        _dailyTrending = List.generate(3, (i) => TrendingStory(id: 'daily-$i', coverImageUrl: 'assets/images/cover_$i.jpg', title: 'Daily Story ${i+1}', views: (i+1) * 1234));
        _weeklyTrending = List.generate(3, (i) => TrendingStory(id: 'weekly-$i', coverImageUrl: 'assets/images/cover_${i+1}.jpg', title: 'Weekly Story ${i+1}', views: (i+1) * 2345));
        _monthlyTrending = List.generate(3, (i) => TrendingStory(id: 'monthly-$i', coverImageUrl: 'assets/images/cover_${i+2}.jpg', title: 'Monthly Story ${i+1}', views: (i+1) * 3456));
        _latestUpdates = List.generate(10, (i) => LatestUpdate(id: 'update-$i', coverImageUrl: 'assets/images/cover_${i % 5}.jpg', title: 'Updated Story ${i+1}', chapter: 'Chapter ${i+15}', time: '${i+1}h ago'));
        _isLoading = false;
      });
  ...
  // --- UPDATED: Helper function to navigate to the detail screen ---
  ...
  Widget _buildContent() { /* ... unchanged ... */ return RefreshIndicator(onRefresh: _fetchHomePageData,color: Colors.white,backgroundColor: Colors.grey[800],child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start,children: [_buildSectionTitle('Newly Added'),_buildCarouselSection(_newlyAddedStories),const SizedBox(height: 20),_buildSectionTitle('Trending Now'),_buildTrendingSection(),const SizedBox(height: 20),_buildLatestUpdateSection(context, _latestUpdates),],),),);}
    ...
    return SizedBox(
      ...
      child: PageView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          // --- UPDATED: Carousel items are now tappable ---
...
// --- ABSTRACTED LIST ITEM WIDGETS (NOW WITH onTap) ---
...
class TrendingListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      onTap: onTap, // --- UPDATED: Make the ListTile tappable ---
      leading: ClipRRect(
